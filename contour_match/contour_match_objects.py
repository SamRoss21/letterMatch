...
def contour_match(img1, img1_id, img2, img2_id, rot=''):
	#bitwise and the contour_shape
	bit_and = cv2.bitwise_and(img1, img2)
	bit_or = cv2.bitwise_or(img1, img2)

	name_and = img1_id + '_' + img2_id + rot + '_and.png'
	cv2.imwrite(output_folder2 + name_and, bit_and)

	name_or = img1_id + '_' + img2_id + rot + '_or.png'
...
def test_image_to_image_rotate():
	img_path = './test_img/'
	img_list = ['bell.jpg','bowtie_macaroni.jpg','candy_cane.jpg','cheese_v.jpg','christmas_candle.jpg',
				'christmas_sock.jpg','christmas_tree.jpg','curved_macaroni.jpg','ginger_bread_man.jpg',
				'mac_plate_o.jpg','macaroni.jpg','ornament.jpg','pine_cone.jpg','pizza_A.jpg','pizza_V.jpg',
	...
	for i in range(len(img_list)):
		...
		for j in range(i+1,len(img_list)):
			...
			for k in range(len(rotations)):
				similarity = contour_match(img1_resized, img1_id, img2_resized, img2_id, "_"+rotations[k])
				pair = {'match': similarity, 'img1': img1_id, 'img2':img2_id+'_'+rotations[k]}
				rotate_results.append(pair)
				img2_resized = cv2.rotate(img2_resized, cv2.ROTATE_90_CLOCKWISE)

			sorted_rotate = sorted(rotate_results, key = lambda k: k['match'], reverse=True)
			results.append(sorted_rotate[0])
